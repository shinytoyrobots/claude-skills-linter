...
describe('lint command — integration', () => {
  ...
  it('discovers .md files and reports validation results', () => {
    ...
    // Should report 48 files checked (.md files minus README.md ignored by default)
  ...
  it('filters files matching ignore patterns', () => {
    ...
    // With 49 .md files, we expect 48 after filtering.
    const { stdout } = run(`lint ${FIXTURES}`);
    assert.ok(stdout.includes('48 files checked'), `expected 48 files (README.md ignored): ${stdout}`);
  });
