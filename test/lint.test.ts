...
describe('lint command — integration', () => {
  ...
  it('discovers .md files and reports validation results', () => {
    ...
    // Should report 43 files checked (.md files minus README.md ignored by default)
    assert.ok(stdout.includes('43 files checked'), `expected "43 files checked" in: ${stdout}`);
  ...
  it('filters files matching ignore patterns', () => {
    ...
    // With 44 .md files, we expect 43 after filtering.
    const { stdout } = run(`lint ${FIXTURES}`);
    assert.ok(stdout.includes('43 files checked'), `expected 43 files (README.md ignored): ${stdout}`);
  });
