...
describe('lint command — integration', () => {
  ...
  it('discovers .md files and reports validation results', () => {
    ...
    // Should report 53 files checked (.md files minus README.md ignored by default)
  ...
  it('filters files matching ignore patterns', () => {
    ...
    // With 54 .md files, we expect 53 after filtering.
    const { stdout } = run(`lint ${FIXTURES}`);
    assert.ok(stdout.includes('53 files checked'), `expected 53 files (README.md ignored): ${stdout}`);
  });
