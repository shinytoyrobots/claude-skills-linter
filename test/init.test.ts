...
describe('runInit', () => {
  ...
  it('AC-5: --force overwrites existing config', () => {
    ...
    assert.equal(exitCode, 0);
    const content = readFileSync(configPath, 'utf-8');
    assert.ok(content.includes('# .skill-lint.yaml — generated by claude-skill-lint init'));
  ...
  it('AC-6: prints success message to stderr', () => {
    ...
    try {
      ...
      assert.ok(output.includes('Run `claude-skill-lint lint .` to validate your files.'));
    } finally {
      process.stderr.write = origWrite;
    }
  });
