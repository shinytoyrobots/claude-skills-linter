...
const VALID_FORMATS: readonly RepoFormat[] = ['legacy-commands', 'plugin', 'multi-plugin', 'project-skills'];
 ...
 */
export function detectFormat(rootDir: string, config: Config): RepoFormat {
  // AC-4 / AC-9: Config override takes precedence
  if (config.format !== undefined) {
    if (!VALID_FORMATS.includes(config.format)) {
      throw new ConfigError(
        `Invalid format "${config.format}" in .skill-lint.yaml. Valid values: ${VALID_FORMATS.join(', ')}`,
      );
    }
  ...
  if (hasMarketplace) {
    ...
    try {
      ...
      process.stderr.write(
        `Warning: .claude-plugin/marketplace.json contains malformed JSON, skipping plugin detection\n`,
      );
  ...
  process.stderr.write(
    `Warning: No recognized repo format signals found, defaulting to legacy-commands\n`,
  );
  return 'legacy-commands';
}
 ...
 */
function hasMultiPlugin(rootDir: string): boolean {
...
function hasProjectSkills(rootDir: string): boolean {
...
function hasSkillFiles(rootDir: string): boolean {
